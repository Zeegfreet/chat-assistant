...
beforeEach( async () => {
    ...
    const testRoles: Role[] = [
        ...
        },
        {
            id: 2,
            role: "Mock Role",
            description: "This is not a mock role",
            isActive: true,
            isDefault: false,
            permissions: [],
            createdAt: new Date(),
        ...
        },
        {
            id: 3,
            role: "Inactive Role",
            description: "This is a inactive role",
            isActive: false,
            isDefault: false,
            permissions: [],
            createdAt: new Date(),
        ...
        id: number + 3,
        role: `Other role ${number}}`,
        description: `This is other role ${number}}`,
        isActive: true,
        isDefault: false,
        permissions: [] as [],
        createdAt: new Date(),
    ...
    await repository.insert(testRoles);
    testLength = testRoles.length;
});
...
const sut = (data: Record<string | number, any>) => request.get("/admin/role")
    .query(data)
    .set("Authorization", `Bearer ${accessToken}`);

/* ---------- TEST ---------- */

describe("Testa rota /ROLE metodo GET", () => {
    describe("Casos de sucesso", () => {
        ...
        test("Deve listar todas roles pesquisadas", async () => {
            const params = {};
        ...
        test("Deve filtrar usando search corretamente", async () => {
            ...
            const data: Role[] = res.body.resources.data;

            const responseRoles = data.map(role => role.role);

            expect(res.statusCode).toBe(200);
            expect(responseRoles).toContain("Mock Role");
            expect(responseRoles).not.toContain("Test Role");
        });
        ...
        test("Deve filtrar usando filter estritamente", async () => {
            ...
            const resources = data.map(item => item.role);

            expect(res.statusCode).toBe(200);
            expect(resources).toContain("Test Role");
            expect(resources).not.toContain("Inactive Role");
        ...
        test("Deve respeitar o operador $gt", async () => {
            ...
            const items: Role[] = res.body.resources.data;

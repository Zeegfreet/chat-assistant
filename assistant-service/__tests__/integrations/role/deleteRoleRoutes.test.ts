...
beforeEach( async () => {
    ...
    const testPermission = roleRepository.create({
        id: 1,
        role: "Tester",
    ...
    const testPermission2 = roleRepository.create({
        id: 2,
        role: "Tester 2",
    ...
    await roleRepository.save(testPermission);
    await roleRepository.save(testPermission2);
...
const sut = (id: string | number) => request.delete(`/admin/role/${id}`)
    .set("Authorization", `Bearer ${accessToken}`);
/* ---------- TESTS --------- */

describe("Testa rota /ROLE método DELETE", () => {
    describe("Casos de sucesso", () => {
        ...
        test("Deve persistir a alteração no banco", async () => {
            ...
            const findByPkRole = await repo.findOneBy({ id });

            expect(findByPkRole).toBeNull();
        });
    });
